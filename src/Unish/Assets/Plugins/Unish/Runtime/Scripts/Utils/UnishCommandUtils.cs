...
namespace Rili.Debug.Shell
{
    public static class UnishCommandUtils
    {
        ...
        private static string ParseVariables(string input, UnishEnvSet env, bool isInsideDoubleQuote)
        {
            ...
            for (var i = 0; i < input.Length; i++)
            {
                ...
                if (c == '\'')
                {
                    ...
                    if (endQuote >= 0)
                    {
                        ...
                        // 外側のダブルクォート内部ならパース
                        if (isInsideDoubleQuote) insideQuote = ParseVariables(insideQuote, env, true);
                ...
                // $の次に空白がある場合は無視
                if (char.IsWhiteSpace(input[i + 1])) continue;
            ...
            if (appendBegin < input.Length) sb.Append(input.Substring(appendBegin));
        ...
        public static List<(string Token, UnishCommandTokenType Type)> CommandToTokens(string input)
        {
            ...
            var tokens = ret.Select(t =>
            {
                ...
                {
                    if (t.Length > 1 && t[1] == '-')
                        type = UnishCommandTokenType.OptionLong;
                    else
                        type = UnishCommandTokenType.OptionShort;
            ...
            for (; i < input.Length; i++)
            {
                ...
                switch (c)
                {
                    ...
                    case var space when char.IsWhiteSpace(space):
                        ...
                        else if (firstInStringSpaceIndex < 0) firstInStringSpaceIndex = i;
                ...
                {
                    if (tokenStartedIndex < 0) tokenStartedIndex = i;
                }
            }
