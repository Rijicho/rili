...
namespace Rili.Debug.Shell
{
    ...
    {
        private const string prefabResourcePath = "Unish/Prefabs/Unish";
        ...
        private DefaultDisplay mDisplay;
        ...
        public async UniTask InitializeAsync()
        {
            ...
            mDisplay = mInstantiated.GetComponent<DefaultDisplay>();
            mBackground  = mDisplay.Background;
            mDisplayText = mDisplay.Text;
            if (mFont)
            {
                mDisplayText.font = mFont;
            }

            mBackground.color     = BuiltInEnv.Get(UnishBuiltInEnvKeys.BgColor, mColorParser.Parse("#000000cc"));
            ...
            mDisplayText.fontSize = BuiltInEnv.Get(UnishBuiltInEnvKeys.FontSize, 24);
        ...
        private void OnEnvSet(UnishVariable envvar)
        {
            switch (envvar.Name)
            {
                case UnishBuiltInEnvKeys.BgColor:
                    mBackground.color = envvar.CastOr(mColorParser.Parse("#000000cc"));
                    break;
                ...
                case UnishBuiltInEnvKeys.FontSize:
                    mDisplayText.fontSize = envvar.CastOr(24);
                    RefleshSize();
                    break;
            }
        }
        ...
        private void OnEnvRemoved(string key)
        {
            ...
            {
                case UnishBuiltInEnvKeys.BgColor:
                    mBackground.color = mColorParser.Parse("#000000cc");
                    break;
                case UnishBuiltInEnvKeys.CharCountPerLine:
        ...
        {
            var prevText    = mDisplayText.text;
            ...
            mDisplayText.text                    = placeHolder.ToString();
            mDisplayText.rectTransform.sizeDelta = new Vector2(mDisplayText.preferredWidth, mDisplayText.preferredHeight);
            mBackground.rectTransform.sizeDelta  = new Vector2(mDisplayText.preferredWidth + 20, mDisplayText.preferredHeight + 20);
            mDisplayText.text                    = prevText;
        }
        ...
        private void UpdateDisplay()
        {
            ...
            if (mSubmittedLines.Count == 0)
            {
                mDisplayText.text = mIsReading ? inputWithCursor : "";
                return;
            }

            if (mDisplayLineOffset == 0)
            {
                mDisplayText.text = mSubmittedLines
