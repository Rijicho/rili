...
using System.Text;
using static Rili.Debug.Shell.UnishPathConstants;
...
namespace Rili.Debug.Shell
{
    public static class UnishPathUtils
    {
        ...
        public static string ConvertToAbsolutePath(string input, string pwd, string homePath)
        {
            ...
            if (input == Home || input == HomeRelativePrefix) return homePath ?? Root;

            if (input == CurrentDir || input == CurrentRelativePrefix) return pwd;

            if (input == ParentDir || input == ParentRelativePrefix)
            {
                if (pwd == Root) return Root;
            ...
            {
                var c = input[i];
                if (c != Separator && c != AltSeparator && i != input.Length - 1) continue;
                ...
                switch (node)
                {
                    ...
                    case Home:
                        pathStack.Clear();
                        if (!isInRoot) pathStack.Push(homePath);
            ...
            // Root
            if (pathStack.Count == 0) return Root;
        ...
        {
            if (path == null || string.IsNullOrWhiteSpace(path) || path == Root) yield break;
